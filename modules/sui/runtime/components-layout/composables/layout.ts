...
export const SUILayoutKey: InjectionKey<LayoutProvide> =
  Symbol.for('sui:layout');
export const SUILayoutItemKey: InjectionKey<{ id: string }> =
  Symbol.for('sui:layout-item');
...
export function useLayout() {
  const layout = inject(SUILayoutKey);

  if (!layout) throw new Error('[SUI] Could not find injected layout');
...
export function createLayout(props: {
  ...
  const parentLayout = inject(SUILayoutKey, null);
  ...
  provide(SUILayoutKey, {
    register: (
      ...
      const layoutItemStyles = computed<CSSProperties>(() => {
        ...
        if (!item) throw new Error(`[SUI] Could not find layout item "${id}"`);
