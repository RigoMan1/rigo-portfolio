...
export function useScroll(props: ScrollProps, args: ScrollArguments = {}) {
  ...
  const scrollRatio = computed(() => {
    return clamp(
      (scrollThreshold.value - currentScroll.value) / scrollThreshold.value || 0
    );
  });
  ...
  const onScroll = () => {
    ...
    currentScroll.value =
      'window' in targetEl ? targetEl.pageYOffset : targetEl.scrollTop;

    isScrollingUp.value = currentScroll.value < previousScroll;
    currentThreshold.value = Math.abs(
