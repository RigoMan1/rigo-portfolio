...
import { makeVSlideProps } from './VSlidesMeta';
...
// Types
import { VSlidesGroupSymbol, VSlidesSymbol } from './VSlidesMeta';
...
if (!slide || !groupItem)
  throw new Error('[SUI] VSlide must be used inside VSlides');
...
function onBeforeTransition() {
  ...
  if (slide.transitionCount.value === 0) {
    ...
    slide.transitionHeight.value = convertToUnit(
      slide.rootRef.value?.clientHeight
    );
  }
...
const transition = computed(() => {
  const name = slide.isReversed.value
